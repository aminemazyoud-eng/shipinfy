...
export default function DeliveryHeatmapInner({ data }: { data: HeatmapData }) {
  ...
  useEffect(() => {
    ...
    async function initMap() {
      ...
      // Center: calculate from heatmap points or fallback to Marrakech center
      ...
      // OpenStreetMap tile layer — 100% gratuit, zéro API key
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>',
        maxZoom: 19,
      }).addTo(map)

      // Heatmap layer (delivered orders)
      if (data.heatmapPoints.length > 0) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
